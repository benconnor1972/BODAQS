...
def test_roundtrip_ops():
    ...
    assert (
        rt("rear_shock [mm]_op_zeroed_Butterworth_2p5Hz_4Order")
        == "rear_shock [mm]_op_zeroed_Butterworth_2p5Hz_4Order"
    )

def test_butterworth_residual_op_chain_maps_to_diff():
    parts = parse_signal_name(
        "rear_shock [mm]_op_zeroed_Butterworth_2p5Hz_4Order_resid",
        DEFAULT_SPEC,
    )
    assert list(parts.ops) == ["zeroed", "diff"]
...
def test_butterworth_op_malformed_rejected():
    with pytest.raises(SignalNameError):
        parse_signal_name("rear_shock [mm]_op_Butterworth_2p5Hz_bad", DEFAULT_SPEC)
