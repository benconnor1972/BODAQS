...
- `session_id` **must be present** and identifies the session that produced this table
- `(session_id, event_id)` **must be globally unique** within the table
...
| `session_id` | `str` | Stable identifier of the source session that produced this event |
...
### 4.2 Optional derived `event_uid`

Producers MAY include:

| Column | Type | Description |
|------|------|-------------|
| `event_uid` | `str` | Convenience global identifier, e.g. `{session_id}:{event_id}` |

Rules:
- If present, `event_uid` must be globally unique
- Consumers must not parse `event_uid`; it is for stable uniqueness only
