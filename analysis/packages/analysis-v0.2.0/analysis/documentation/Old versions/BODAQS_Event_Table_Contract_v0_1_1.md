# BODAQS Event Table Contract (v0.1)
...
| Column | Type | Meaning |
|------|------|--------|
| `signal` | str | Primary signal role used for detection (schema terminology) |
| `signal_col` | str | Concrete dataframe column name actually used (resolved from session signal registry) |

### Optional / Future-proof

| Column | Type | Meaning |
|------|------|--------|
| `signals` | list[str] | All signals (roles) involved in the event (if applicable) |

### Notes

- `signal` maps directly to schema signal roles (e.g. `disp`, `vel`, `acc`, `disp_zeroed`, `disp_norm`).
- `signal_col` MUST be a column present in `session["df"]`.
- Signal resolution is performed upstream via signal standardisation + the session signal registry (`session["meta"]["signals"]`).
...
### Segment extraction (downstream)

**Important:** `segment_id` in this table refers to *ride/session segmentation* (a classifier label attached to an event at detection time).
It is **not** the same thing as a *segment extractor* window (Recommendation #5), which is produced **after** detection.

Downstream segment extraction uses the event table's time/index anchors (`start_*`, `trigger_*`, `end_*`) plus the signal registry in `session['meta']['signals']`.
Extraction policy (default anchor/window/roles) may be provided in the **event schema** under an optional `segment_defaults` block; this does **not** change the event table columns.
