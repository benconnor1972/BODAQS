...
<Aside type={"caution"} title="These materials are for the BODAQS A8 logger"  icon="seti:config">
    The following materials work for the BODAQS A8 logger, currently version 1.0. If you are building the Prototype F logger, start [here]()
</Aside>
...
Go to the 'releases' page in the BODAQS github repository [here](https://github.com/benconnor1972/BODAQS/tree/main/hardware/releases) and download the latest PCB fabrication package. The package contains all the files you need to order an assembled PCB from JLCPCB.
...
<Flag src="/src/assets/pics/logger-build-guide/hardware_3.png" alt="hardware_3">
...
<Flag src="/src/assets/pics/logger-build-guide/hardware_4.png" alt="hardware_4">
...
<Flag src="/src/assets/pics/logger-build-guide/hardware_5.png" alt="hardware_5">
You will be presented with the Bill of Materials and Component Placements. Click 'next' on each page.
</Flag>

<Flag src="/src/assets/pics/logger-build-guide/hardware_6.png" alt="hardware_6">
...
<Aside title={'Ordering tips'}>
 ...
 - In the future, we may set up a storefront to allow you to order the PCB and other components directly. We don't intend to do this until the hardware design is stable. Note also that ordering from the storefront will be a premium option: stocking, handling, postage etc take time. If you're after the lowest price, sourcing the parts yourself is likely to be the cheapest option.
</Aside>
...
<Lightbox src="/src/assets/pics/logger-build-guide/OLED_dims.png" alt="OLED dims" />


#### Nintendo DS2 replacement membranes **
<Flag src="/src/assets/pics/logger-build-guide/keypad.png" alt="keypad ">
...
#### Li-ion battery
<Flag src={"/src/assets/pics/logger-build-guide/battery.png"} alt="battery">
  The Li-ion battery is xxx size, with JST-PH 2 pin connector. xxx Li-ion cells are widely available as replacement parts from AliExpress, Amazon, Ebay and local vendors. For our purposes, all are equally good. However, make sure that:
    - The battery's maximum dimensions are xxx  x yyy x zzz
...
The BODAQS A8 logger has 4 external connectors: two for analog inputs, one for I2C and one connector for a handlebar-mounted switch and GPS connection. All the connectors are identical 6-pin metal M8 circular connectors. Amphenol xxx

<Lightbox src="/src/assets/pics/logger-build-guide/male_3_pin.png" alt="male 3 pin " />
<Lightbox src="/src/assets/pics/logger-build-guide/male_4_pin.png" alt="male 4 pin " />

<Aside  title="Optional">
  - The **premium** option for connectors is to use original Amphenol connectors.
...
<Flag src="/src/assets/pics/logger-build-guide/plugs.png" alt="plugs">
...
All the print files are available [here](https://github.com/benconnor1972/BODAQS/tree/main/mechanical/Case/a8%v1_0). (The Fusion CAD model of the case is in the 'f3z' directory). 

### Parts required

You will need the following printed parts:
 - **A base** (the lower half of the case). There are two variants of the base design. `base.stl` is designed to be fixed closed using brass heat-set inserts in the lower half of the case. This is cosmetically superior, but requires you to have the inserts. `base_nuts.stl`is to use standard 3mm hex nuts. This works just as well but doesn't look quite so tidy.
...
Bambulab and other vendors sell dedicated support-interface materials, but all you really need is two materials that don't stick well to each other. If you are printing in PLA, you can use PETG as am interface material, and vice versa.
</Aside>
