...
The schema uses **suffix-only resolution**, meaning each sensor name maps to its column suffixes defined in:
...
Thus a sensor `"rear_shock"` yields columns:  
`rear_shock_disp`, `rear_shock_vel`, `rear_shock_acc`.
...
| `delta` | op, value | delta from t0 meets condition |
| `peak` | kind, op, value | peak/minimum meets threshold |
